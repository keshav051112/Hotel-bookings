  ...
  return (
    <div>
      ...
      <div className="flex gap-4 my-8">
        <div className="bg-blue-50 border border-blue-200 rounded flex p-4 pr-8">
          <img
          ...
          <tbody className="text-sm">
            {dashboardData.bookings.map((items,index)=>(
                <tr key={index}>
                    ...
                    <td  className="py-3 px-4 pl-20 text-gray-700 border-t border-gray-300">
                        $ {items.totalPrice}
                    </td>
                    
                    <td className="py-3 px-4 border-t border-gray-300 flex">
                        <button className={`py-1 px-3 text-xs rounded-full mx-auto ${items.isPaid ? 'bg-green-600' : "bg-amber-200 text-yellow-600"}`}>
