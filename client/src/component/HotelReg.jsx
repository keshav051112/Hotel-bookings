...
const HotelReg = () => {
  ...
  const onSubmitHandler = async(event)=>{
    try {
      ...
      const response = await axios.post(`/api/hotels/`,{name,contact,address,city},{headers:{Authorization : `Bearer ${await getToken()}`}})
      ...
      <form onSubmit={onSubmitHandler} onClick={(e)=>e.stopPropagation()} className="flex bg-white rounded-xl max-w-4xl max-md:mx-2">
        ...
        <div className="relative flex flex-col items-center md:w-1/2 p-8 md:p-10">
          ...
          <div className="w-full mt-4">
               <label htmlFor="name" className="font-medium text-gary-500">Hotel Name</label>
             ...
             <div className="w-full mt-4">
               <label htmlFor="contact" className="font-medium text-gary-500">Contact</label>
          ...
          <div className="w-full mt-4">
               <label htmlFor="address" className="font-medium text-gary-500">Address</label>
